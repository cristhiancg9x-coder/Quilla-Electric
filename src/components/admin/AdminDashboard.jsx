...
import { 
  ...
  Menu, 
  Bell,
  User, // <--- Icono para el CRM
  CreditCard
} from 'lucide-react';

// === IMPORTAMOS TODAS TUS HERRAMIENTAS ===
import SaaSManager from './SaaSManager';       // (Legacy)
import ProductManager from './ProductManager'; // Inventario
import ProductUploader from './ProductUploader'; // Base de Datos
import ConfigManager from './ConfigManager';   // Configuración de Precios
import LeadsManager from './LeadsManager';     // <--- NUEVO CRM (Buzonera)
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-white overflow-hidden font-sans">
        ...
        <div className="h-20 flex items-center px-6 border-b border-slate-800">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-[10px] text-slate-500 uppercase tracking-wider font-bold mt-1">Panel Admin v2.2</p>
                </div>
            </div>
        </div>
        ...
        <nav className="p-4 space-y-2 overflow-y-auto h-[calc(100vh-180px)]">
            ...
            {menuItems.map((item) => (
                ...
                >
                    ...
                    {/* Badge para CRM (Opcional: podrías poner un contador real aquí luego) */}
                    {item.id === 'crm' && (
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-slate-950 relative">
        ...
        {/* CONTENIDO DINÁMICO (Aquí cambia la magia) */}
        <div className="flex-1 overflow-y-auto p-4 md:p-8 scroll-smooth">
            <div className="max-w-7xl mx-auto animate-fadeIn pb-20">
                ...
                {/* 3. PESTAÑA CONTROL SAAS */}
                {activeTab === 'saas' && (
                    <div className="space-y-8">
                        
                        {/* Gestor de Costos (Configuración) */}
                        <ConfigManager />

                        {/* SaaS Manager Antiguo (Legacy) */}
